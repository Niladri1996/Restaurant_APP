        ...
        storageReference.putFile(resultUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                if(uriTask.isSuccessful())
                {
                    ...
                    map.put("FoodId",timestamp);
                    map.put("FoodName",ed_pname.getText().toString());
                    map.put("FoodPrice",ed_pprice.getText().toString());
                    map.put("Timestamp",timestamp);
                    map.put("FoodImage",downloadimage);

                    reference.child("FoodsDetails")
                            .child(timestamp).setValue(map).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            progressDialog.dismiss();
                            Toast.makeText(FoodUpload.this, "Product save successfully", Toast.LENGTH_SHORT).show();
                            Intent intent=new Intent(FoodUpload.this,MenuActivity.class);
                        ...
                        public void onFailure(@NonNull Exception e) {
                            progressDialog.dismiss();
                            Toast.makeText(FoodUpload.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
                }
